      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="box">
            ...
            <label for="prodBrand">Производитель</label><br>
  <select name="prodBrand" id="prodBrand" class="form-control" required>
                @foreach($brand as $br)
                <option name={{$br->name}} value= {{$br->id}}>{{$br->name}}</option>
                @endforeach
            </select><br>
          ...
          <div style="overflow: auto;">
            ...
            <tr>
            <td><input type='checkbox' name = "propertyName[]" value="{{$prop->id}}"><label>{{$prop->name}}</label><input name ="{{$prop->id}}"></td>
            </tr>
            @endforeach
          </table>
          </div>
...
<div id = "editProduct" style="display: none">
   <form action={{url('/editProd')}} method="post" class="col-lg-4 col-md-offset-2" id="editProduct" enctype="multipart/form-data">
            <label><h3>Редактировать товар</h3></label><br>

            <label for="prodName">Название товара</label><br>
            <input name="prodNameEdit" id="prodNameEdit" class="form-control input-sm" required><br>
            <label for="prodPrice">Стоимость товара</label><br>
            <input name="prodPriceEdit" id="prodPriceEdit" class="form-control input-sm" pattern="^[ 0-9]+$" required><br>
            <label for="prodCategory">Категория товара</label><br>
            <select name="prodCategoryEdit" id="prodCategoryEdit" class="form-control" required>
                @foreach($category as $cat)
                <option name={{$cat->name}} value= {{$cat->id}}>{{$cat->name}}</option>
                @endforeach
            </select><br>
            <label>Производитель</label>
              <select name="prodBrandEdit" id="prodBrandEdit" class="form-control" required>
                @foreach($brand as $br)
                <option name={{$br->name}} value= {{$br->id}}>{{$br->name}}</option>
                @endforeach
              </select>
            <label for="descriptionProd">Описание товара</label><br>
            <textarea class = "form-control" id = "descriptionProdEdit" name = "descriptionProdEdit"></textarea>
            <img src = "#" id = "imageEdit">
            ...
            <div class="box-body">
              <table class="table table-bordered">
                <tr>
                  ...
                  <th style="width: 30px">Категория</th>
                  <th style="width: 30px">Производитель</th>
                ...
                <tr>
                 ...
                 <div class = "form-row">
                  <div class = "col">
                   <input type = "hidden" id = "idProduct" name = "idProduct" value = "{{$prod->id}}">
                    <button type = "submit" class = "btn btn-primary" value = "X" onclick="showEditProductForm(this)">
                ...
                <div class = "col">
                  <form action = "{{url('/deleteProd')}}" method = "post" style="width:30px">
